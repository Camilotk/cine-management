...
export default function SimpleModal(item) {
  ...
  const okUpdate = async (data) => {
    try{
        ...
        alert('Houve um erro ao editar este filme: '+response.response.data.error)
    }
  ...
  const body = (
    ...
    <div style={modalStyle } className={classes.paper}>
      ...
      <div className="inputItem"><span className="span">Título:</span> <Input required defaultValue={item.titulo} name="titulo" type="text" /></div>
      

      
      <div className="inputItem"><span className="span">Descrição:</span>  <Input required defaultValue={item.descricao} name="descricao" type="text" /></div>

      <div className="inputItem"><span className="span">Duração:</span>  <Input required defaultValue={item.duracao} name="duracao" type="text" /></div>
      <div style={{
          display:'flex',
  ...
  const body2 = (
    ...
    <div style={ modalStyle } className={classes.paper}>
      <h2 id="simple-modal-title">{item.titulo} foi editado com sucesso!</h2>
      <br/>
      <div style={{
          display:'flex',
